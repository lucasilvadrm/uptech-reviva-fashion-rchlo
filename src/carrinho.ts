...
const ulSacola = criaElemento(TagName.ul, [], '', ['wallet__list'], secaoSacola as HTMLElement);

const valorTotal = document.querySelector('.total__value');

const ulQuantidadePreco = document.querySelector('.quantity-price') as HTMLElement;
...
carrinho.forEach((item, indice) => {
    ...
    criaElemento(TagName.img,
        [{ nome: 'src', valor: item.imagens[0].url },
        { nome: 'alt', valor: '' }
        ], '', [], divImagemProduto);
    ...
    for (let i = 0; i < 3; i++) {
        ...
        criaElemento(TagName.label,
            [{ nome: 'for', valor: `size-${tamanho}${indice + 1}` }], tamanho.toUpperCase(), ['size__label'], tamanhoProduto);
    ...
    const inputQuantidade = document.getElementById(`quantity${indice}`) as HTMLSelectElement;
...
function atualizaCarrinho(
    ...
    elemento.addEventListener('change', event => {
        ...
        let novaQuantidadeCarrinho = {
            ...item,
            quantidade_carrinho: Number(valor.value),
