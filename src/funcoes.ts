import { Atributo, Produto } from "./interfaces";

export function criaElemento(tag: string, atributos: Atributo[], conteudo: string, classes: string[], elementoPai: HTMLElement): HTMLElement {
...
export function calculaTotalCarrinho(carrinho: Produto[]): string {
    const total = carrinho.reduce((acm, item) => acm + (item.quantidade_carrinho * item.preco), 0);
    return formatarPreco(total, true);
}

export function verificaLocalStorage(estoque: Object): void {
